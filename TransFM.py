# Implementation of the TransFM model
import pandas as pd
...
class TransFM:
    ...
    def get_preds(self, var_linear, var_emb_factors, var_trans_factors,
        ...
        var_trans_product = tf.reduce_sum(tf.square(var_trans_factors), axis=1, keep_dims=True)
        var_emb_trans_product = tf.reduce_sum(tf.multiply(var_emb_factors, var_trans_factors),
        ...
        pos_trans_mul = tf.sparse_tensor_dense_matmul(sparse_pos_feats, var_trans_factors)
        ...
        neg_trans_mul = tf.sparse_tensor_dense_matmul(sparse_neg_feats, var_trans_factors)
        ...
        # Term 2 pos
        prod_term_pos = tf.sparse_tensor_dense_matmul(
                sparse_pos_feats, var_trans_product)
        ...
        # Term 4 pos
        prod_term_pos = tf.sparse_tensor_dense_matmul(
                sparse_pos_feats, var_emb_trans_product)
        term_4_pos = 2 * prod_term_pos * pos_feats_sum

        # Term 4 neg
        prod_term_neg = tf.sparse_tensor_dense_matmul(
                sparse_neg_feats, var_emb_trans_product)
        term_4_neg = 2 * prod_term_neg * neg_feats_sum

        # Term 5
        term_5_pos = 2 * tf.reduce_sum(tf.square(pos_emb_mul), axis=1, keep_dims=True)
        term_5_neg = 2 * tf.reduce_sum(tf.square(neg_emb_mul), axis=1, keep_dims=True)

        # Term 6
        term_6_pos = 2 * tf.reduce_sum(pos_trans_mul * pos_emb_mul, axis=1, keep_dims=True)
        term_6_neg = 2 * tf.reduce_sum(neg_trans_mul * neg_emb_mul, axis=1, keep_dims=True)

        # Diag term
        diag_term_pos = tf.reduce_sum(tf.square(pos_trans_mul), axis=1, keep_dims=True)
        diag_term_neg = tf.reduce_sum(tf.square(neg_trans_mul), axis=1, keep_dims=True)

        # Predictions
        pos_preds = pos_linear + 0.5 * (
                term_1_pos + term_2_pos + term_3_pos
    ...
    def create_model(self):
        ...
        with g.as_default():
            ...
            pos_preds, neg_preds = self.get_preds(var_linear, var_emb_factors,
                    var_trans_factors, sparse_pos_feats, sparse_neg_feats)

            l2_reg = tf.add_n([
                self.args.linear_reg * tf.reduce_sum(tf.square(var_linear)),
                self.args.emb_reg    * tf.reduce_sum(tf.square(var_emb_factors)),
                self.args.trans_reg  * tf.reduce_sum(tf.square(var_trans_factors)),
            ])
